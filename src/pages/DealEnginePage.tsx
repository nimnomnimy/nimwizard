...
export default function DealEnginePage() {
  ...
  const right = useResizable({ initial: 300, min: 200, max: 500 })
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-slate-50">
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden select-none">
          ...
          {left.isOpen ? (
            <>
              <div style={{ width: left.width }} className="flex-shrink-0 border-r border-slate-200 bg-white flex flex-col overflow-hidden">
                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-500">Deals ({sortedDeals.length})</span>
                ...
                <div className="flex-1 overflow-y-auto py-1">
                  {sortedDeals.length === 0 && (
                    <p className="text-xs text-slate-400 text-center py-6 px-3">No deals yet.<br/>Click "+ New Deal" above.</p>
                  )}
                  {sortedDeals.map(d => (
              ...
              <div {...left.dragHandleProps} className="w-1.5 flex-shrink-0 cursor-col-resize hover:bg-blue-400 bg-transparent hover:transition-colors group">
                  ...
                  <div className="flex flex-col gap-2 mb-4">
                    {activeDeal.lineItems.length === 0 && (
                      <div className="bg-white border border-slate-200 border-dashed rounded-xl py-8 text-center">
                        ...
                        <button onClick={addLineItem} className="mt-2 text-sm text-blue-500 hover:text-blue-700 font-semibold">
                    ...
                    {activeDeal.lineItems.length > 0 && (
                      <button onClick={addLineItem}
                        className="flex items-center gap-1.5 text-sm text-blue-500 hover:text-blue-700 font-semibold py-1 transition-colors">
          ...
          {activeDeal && metrics && (
            right.isOpen ? (
              <>
                <div {...right.dragHandleProps} className="w-1.5 flex-shrink-0 cursor-col-resize group">
                  <div className="w-px h-full bg-slate-200 group-hover:bg-blue-400 mx-auto transition-colors" />
                </div>
                <div style={{ width: right.width }} className="flex-shrink-0 bg-white flex flex-col overflow-hidden border-l border-slate-200">
                  <div className="flex items-center border-b border-slate-200 flex-shrink-0">
                    ...
                    <button onClick={() => right.setIsOpen(false)} className="px-2 text-slate-300 hover:text-slate-500" title="Collapse">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M4 2l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-3">
                    ...
                    {rightTab === 'optimizer' && (
                      <OptimizationPanel deal={activeDeal} products={products} onApplyRecommendation={handleApplyRecommendation} onApplyAll={handleApplyAll} />
                    )}
                    {rightTab === 'charts' && (
              ...
              <button onClick={() => right.setIsOpen(true)}
                className="w-7 flex-shrink-0 border-l border-slate-200 bg-white hover:bg-slate-50 flex items-center justify-center text-slate-300 hover:text-slate-500 transition-colors"
